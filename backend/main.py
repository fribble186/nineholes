...
def ai_process(state):
    ...
    environment = AiUtils.init_environment()
    white_q_matrix = np.load("q_matrix.npy")
    white_r_matrix = np.load("r_matrix.npy")
    black_p_matrix = np.load("p_matrix.npy")
    # AiUtils.test(white_q_matrix)

    black_r_matrix = AiUtils.init_black_return_matrix(environment)
    ...
    if win_index == -1:
        ...
        for available in available_list:
            ...
            for black_available in available["black_available_list"]:
                count += black_p_matrix[available["black_state_index"], black_available]
            for black_available in available["black_available_list"]:
                white_r_next_list = white_r_matrix[black_available]
                white_q_next_list = white_q_matrix[black_available]
                ...
                if len(q_available_list) == 0:
                    available["q"] += -black_p_matrix[available["black_state_index"], black_available] / count
                else:
                    available["q"] += (black_p_matrix[available["black_state_index"], black_available] / count) * max(
                        q_available_list)
        if len(available_list) > 0:
            ...
            next_state = environment[next_states[next_state_index]["target_action_index"]]
        ...
        next_state = environment[win_index]
        for index_1 in range(3):
            for index_2 in range(3):
                index = index_1 * 3 + index_2
                if index in next_state["black"]:
